...
export default function EveryDayBasics() {
  const [sortOption, setSortOption] = useState("");
  const [showSortDropdown, setShowSortDropdown] = useState(false);
  const { data, originalData, setData } = useContext(ProductsContext);

  const everyDayBasicsProducts = useMemo(() => {
    if (!data || !Array.isArray(data.everyDayBasicsProducts)) return [];
    return data.everyDayBasicsProducts;
  }, [data]);
    ...
    >
      <>
        ...
        <Wrapper>
          ...
          />
          <Items data={everyDayBasicsProducts} />
        </Wrapper>
      </>
      <Footer />
    </MotionWrapper>
